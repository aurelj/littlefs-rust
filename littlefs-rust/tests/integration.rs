...
async fn test_write_read_roundtrip() {
    ...
    let file = fs.open("/hello.txt", OpenFlags::READ).await.unwrap();
...
async fn test_seek_tell_size() {
    ...
    let file = fs.open("/data.bin", OpenFlags::READ).await.unwrap();
...
async fn test_truncate() {
    ...
    let file = fs.open("/trunc.txt", OpenFlags::WRITE).await.unwrap();
