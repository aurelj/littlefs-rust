...
#[tokio::test]
async fn test_evil_invalid_tail_pointer() {
    for &tail_type in &[LFS_TYPE_HARDTAIL, LFS_TYPE_SOFTTAIL] {
        for &invalset in &[0x3u32, 0x1, 0x2] {
            unsafe { evil_invalid_tail_pointer(tail_type, invalset).await };
        }
    }
}

async unsafe fn evil_invalid_tail_pointer(tail_type: u32, invalset: u32) {
    ...
    let pair: [u32; 2] = [0, 1];
    assert_ok(lfs_dir_fetch(&mut lfs, &mut caches, mdir.as_mut_ptr(), &pair).await);
    ...
    assert_err(LFS_ERR_CORRUPT, lfs_mount(&mut lfs, &mut caches, cfg).await);
}
...
async fn test_evil_invalid_dir_pointer() {
    for &invalset in &[0x3u32, 0x1, 0x2] {
        unsafe { evil_invalid_dir_pointer(invalset).await };
    }
}

async unsafe fn evil_invalid_dir_pointer(invalset: u32) {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
    ...
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, dir_name.as_ptr()).await);
    ...
    assert_ok(lfs_stat(&mut lfs, &mut caches, dir_name.as_ptr(), info.as_mut_ptr()).await);
    ...
    assert_err(
        LFS_ERR_CORRUPT,
        lfs_dir_open(&mut lfs, &mut caches, dir.as_mut_ptr(), dir_name.as_ptr()).await,
    );
    ...
    assert_err(
        LFS_ERR_CORRUPT,
        lfs_dir_open(&mut lfs, &mut caches, dir.as_mut_ptr(), child_dir.as_ptr()).await,
    );
...
#[tokio::test]
async fn test_evil_invalid_file_pointer() {
    for &size in &[10u32, 1000, 100000] {
        unsafe { evil_invalid_file_pointer(size).await };
    }
}

async unsafe fn evil_invalid_file_pointer(size: u32) {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
    ...
    let mut info = core::mem::MaybeUninit::<LfsInfo>::zeroed();
    assert_ok(lfs_stat(&mut lfs, &mut caches, file_name.as_ptr(), info.as_mut_ptr()).await);
    let info_ref = &*info.as_ptr();
    let nul = info_ref.name.iter().position(|&b| b == 0).unwrap_or(256);
    ...
    assert_err(
        LFS_ERR_CORRUPT,
        lfs_file_read(&mut lfs, &mut caches, file.as_mut_ptr(), &mut buffer).await,
    );
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()).await);
    ...
    if size > 2 * BLOCK_SIZE {
        ...
        assert_err(
            LFS_ERR_CORRUPT,
            lfs_mkdir(&mut lfs, &mut caches, dir_name.as_ptr()).await,
        );
    }
...
#[tokio::test]
async fn test_evil_invalid_ctz_pointer() {
    for &size in &[2 * BLOCK_SIZE, 3 * BLOCK_SIZE, 4 * BLOCK_SIZE] {
        unsafe { evil_invalid_ctz_pointer(size).await };
    }
}

async unsafe fn evil_invalid_ctz_pointer(size: u32) {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
    ...
    for _ in 0..size {
        let n = lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"c").await;
        assert_eq!(n, 1);
    }
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()).await);
    assert_ok(lfs_unmount(&mut lfs));
    ...
    assert!(lfs.storage.read(ctz.head, 0, &mut bbuffer).await.is_ok());
    let bad: u32 = lfs_tole32(0xcccccccc);
    bbuffer[0..4].copy_from_slice(&bad.to_ne_bytes());
    bbuffer[4..8].copy_from_slice(&bad.to_ne_bytes());
    assert!(lfs.storage.erase(ctz.head).await.is_ok());
    assert!(lfs.storage.write(ctz.head, 0, &bbuffer).await.is_ok());
    assert_ok(lfs_deinit(&mut lfs));
...
async fn test_evil_invalid_gstate_pointer() {
    for &invalset in &[0x3u32, 0x1, 0x2] {
        unsafe { evil_invalid_gstate_pointer(invalset).await };
    }
}

async unsafe fn evil_invalid_gstate_pointer(invalset: u32) {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
...
#[tokio::test]
async fn test_evil_mdir_loop() {
    unsafe { evil_mdir_loop().await };
}

async unsafe fn evil_mdir_loop() {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
...
#[tokio::test]
async fn test_evil_mdir_loop2() {
    unsafe { evil_mdir_loop2().await };
}

async unsafe fn evil_mdir_loop2() {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
    ...
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, child.as_ptr()).await);
    assert_ok(lfs_unmount(&mut lfs));
    ...
    assert_ok(lfs_dir_fetch(&mut lfs, &mut caches, mdir.as_mut_ptr(), &root_pair).await);
    ...
    assert_ok(lfs_dir_fetch(&mut lfs, &mut caches, mdir.as_mut_ptr(), &child_pair).await);
...
/// with LFS_ERR_CORRUPT.
#[tokio::test]
async fn test_evil_mdir_loop_child() {
    unsafe { evil_mdir_loop_child().await };
}

async unsafe fn evil_mdir_loop_child() {
    let mut env = default_config(BLOCK_COUNT);
    init_context(&mut env);
    let cfg = &env.config as *const LfsConfig;
