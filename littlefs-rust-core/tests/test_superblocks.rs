...
use littlefs_rust_core::{
    ...
    lfs_fs_stat, lfs_mount, lfs_remove, lfs_stat, lfs_unmount, Lfs, LfsCaches, LfsConfig, LfsFile,
    LfsFsinfo, LfsInfo, LFS_ERR_INVAL, LFS_ERR_NOENT,
};
use rstest::rstest;
...
#[tokio::test]
async fn test_superblocks_format() {
...
#[tokio::test]
async fn test_superblocks_mount() {
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
    let mut caches = LfsCaches::default();
...
#[tokio::test]
async fn test_superblocks_magic() {
    common::init_logger();
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
...
#[tokio::test]
async fn test_traverse_filter_gets_superblock_after_push() {
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
    let mut caches = LfsCaches::default();
...
#[tokio::test]
async fn test_superblocks_invalid_mount() {
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
    let mut caches = LfsCaches::default();
...
#[tokio::test]
async fn test_superblocks_stat() {
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
    let mut caches = LfsCaches::default();
...
#[tokio::test]
async fn test_superblocks_mount_unknown_block_count() {
    let mut env = default_config(128);
    init_context(&mut env);
    let mut lfs = Lfs::new(&mut env.ram);
    let mut caches = LfsCaches::default();
    ...
    assert_ok(lfs_mount(&mut lfs, &mut caches, &cfg0.config as *const LfsConfig).await);
...
#[tokio::test]
async fn test_superblocks_stat_tweaked() {
    let mut env = default_config(128);
    init_context(&mut env);
...
#[tokio::test]
async fn test_superblocks_expand() {
    for &block_cycles in &[32i32, 33, 1] {
        for &n in &[10u32, 100, 1000] {
            ...
            for _ in 0..n {
                ...
                assert_ok(lfs_stat(&mut lfs, &mut caches, dummy.as_ptr(), info.as_mut_ptr()).await);
                let info = unsafe { info.assume_init() };
                assert_eq!(info.type_, LFS_TYPE_REG as u8);
                assert_ok(lfs_remove(&mut lfs, &mut caches, dummy.as_ptr()).await);
            }
            assert_ok(lfs_unmount(&mut lfs));
...
#[tokio::test]
async fn test_superblocks_magic_expand() {
    common::init_logger();
    for &block_cycles in &[32i32, 33, 1] {
        for &n in &[10u32, 100, 1000] {
            let mut env = default_config(128);
...
#[tokio::test]
async fn test_superblocks_expand_power_cycle() {
    for &block_cycles in &[32i32, 33, 1] {
        for &n in &[10u32, 100, 1000] {
            let mut env = default_config(128);
            init_context(&mut env);
            ...
            for i in 0..n {
                assert_ok(lfs_mount(&mut lfs, &mut caches, &env.config as *const LfsConfig).await);
                let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                let err = lfs_stat(&mut lfs, &mut caches, dummy.as_ptr(), info.as_mut_ptr()).await;
                assert!(
    ...
    for &block_cycles in &[2i32, 1] {
        ...
        let result = run_powerloss_linear(
            ...
            async |lfs, caches, config| {
                ...
                for i in 0..N {
                    let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                    let err = lfs_stat(lfs, caches, dummy.as_ptr(), info.as_mut_ptr()).await;
                    if err == 0 {
                        let info = unsafe { info.assume_init() };
                        if info.type_ == LFS_TYPE_REG as u8 {
                            let e = lfs_remove(lfs, caches, dummy.as_ptr()).await;
                            if e != 0 {
                    ...
                    let e = lfs_file_close(lfs, caches, file.as_mut_ptr()).await;
                    if e != 0 {
                        let _ = lfs_unmount(lfs);
                        return Err(e);
                    }
                    let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                    let e = lfs_stat(lfs, caches, dummy.as_ptr(), info.as_mut_ptr()).await;
                    if e != 0 {
                        let _ = lfs_unmount(lfs);
                        return Err(e);
                    }
...
#[tokio::test]
async fn test_superblocks_unknown_blocks() {
    ...
    let data = b"hello!";
    assert_eq!(
        lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), data).await,
        data.len() as i32
    );
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()).await);
    assert_ok(lfs_unmount(&mut lfs));
...
#[tokio::test]
async fn test_superblocks_fewer_blocks() {
    ...
    for &block_count in &[ERASE_COUNT / 2, ERASE_COUNT / 4, 2u32] {
        ...
        let err = lfs_mount(&mut lfs, &mut caches, &cfg_full.config as *const LfsConfig).await;
        ...
        assert_eq!(
            lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"hello!").await,
            6
        );
        assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()).await);
        assert_ok(lfs_unmount(&mut lfs));
...
#[tokio::test]
async fn test_superblocks_more_blocks() {
    const ERASE_COUNT: u32 = 128;
    ...
    let err = lfs_mount(&mut lfs, &mut caches, &cfg_half.config as *const LfsConfig).await;
    assert_err(LFS_ERR_INVAL, err);
}
...
#[rstest]
#[tokio::test]
async fn test_superblocks_grow(
    #[values(
        ERASE_COUNT_GROW / 2,
        ERASE_COUNT_GROW / 4,
        2u32
    ...
    assert_eq!(
        buf.len() as i32,
        lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), buf).await
    );
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()).await);

    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, large_count).await);
    assert_ok(lfs_unmount(&mut lfs));
    ...
    let mut rbuf = [0u8; 16];
    let n = lfs_file_read(&mut lfs, &mut caches, file.as_mut_ptr(), &mut rbuf).await;
...
async fn test_superblocks_shrink(
    ...
    // same size is a noop
    assert_ok(lfs_mount(&mut lfs, &mut caches, &env.config as *const LfsConfig).await);
    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, BLOCK_COUNT).await);
    let mut fsinfo = core::mem::MaybeUninit::<LfsFsinfo>::uninit();
    assert_ok(lfs_fs_stat(&mut lfs, &mut caches, fsinfo.as_mut_ptr()).await);
    let fsinfo = unsafe { fsinfo.assume_init() };
    assert_eq!(fsinfo.block_size, BLOCK_SIZE);
    ...
    assert_ok(lfs_mount(&mut lfs, &mut caches, &env.config as *const LfsConfig).await);
    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, block_count_2).await);
    let mut fsinfo = core::mem::MaybeUninit::<LfsFsinfo>::uninit();
    assert_ok(lfs_fs_stat(&mut lfs, &mut caches, fsinfo.as_mut_ptr()).await);
    let fsinfo = unsafe { fsinfo.assume_init() };
    assert_eq!(fsinfo.block_size, BLOCK_SIZE);
    ...
    assert_err(
        LFS_ERR_INVAL,
        lfs_mount(&mut lfs, &mut caches, &cfg_old.config as *const LfsConfig).await,
    );
...
#[rstest]
#[tokio::test]
async fn test_superblocks_metadata_max(
