...
fn test_superblocks_reentrant_format() {
    ...
    let result = run_powerloss_linear(
        ...
        |lfs, caches, config| {
            ...
            if err != 0 {
                let e = lfs_format(lfs, caches, config);
                if e != 0 {
                    return Err(e);
                }
    ...
    for &block_cycles in &[32i32, 33, 1] {
        for &n in &[10u32, 100, 1000] {
            ...
            for _ in 0..n {
                ...
                assert_ok(lfs_remove(&mut lfs, &mut caches, dummy.as_ptr()));
            }
            assert_ok(lfs_unmount(&mut lfs));
    ...
    for &block_cycles in &[32i32, 33, 1] {
        for &n in &[10u32, 100, 1000] {
            ...
            for i in 0..n {
                ...
                let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                let err = lfs_stat(&mut lfs, &mut caches, dummy.as_ptr(), info.as_mut_ptr());
                assert!(
    ...
    for &block_cycles in &[2i32, 1] {
        ...
        let result = run_powerloss_linear(
            ...
            |lfs, caches, config| {
                ...
                for i in 0..N {
                    let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                    let err = lfs_stat(lfs, caches, dummy.as_ptr(), info.as_mut_ptr());
                    if err == 0 {
                        let info = unsafe { info.assume_init() };
                        if info.type_ == LFS_TYPE_REG as u8 {
                            let e = lfs_remove(lfs, caches, dummy.as_ptr());
                            if e != 0 {
                    ...
                    let e = lfs_file_close(lfs, caches, file.as_mut_ptr());
                    if e != 0 {
                        let _ = lfs_unmount(lfs);
                        return Err(e);
                    }
                    let mut info = core::mem::MaybeUninit::<LfsInfo>::uninit();
                    let e = lfs_stat(lfs, caches, dummy.as_ptr(), info.as_mut_ptr());
                    if e != 0 {
                        let _ = lfs_unmount(lfs);
                        return Err(e);
                    }
...
fn test_superblocks_grow(
    ...
    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, large_count));
    assert_ok(lfs_unmount(&mut lfs));
...
fn test_superblocks_shrink(
    ...
    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, BLOCK_COUNT));
    let mut fsinfo = core::mem::MaybeUninit::<LfsFsinfo>::uninit();
    assert_ok(lfs_fs_stat(&mut lfs, &mut caches, fsinfo.as_mut_ptr()));
    let fsinfo = unsafe { fsinfo.assume_init() };
    assert_eq!(fsinfo.block_size, BLOCK_SIZE);
    ...
    assert_ok(lfs_fs_grow(&mut lfs, &mut caches, block_count_2));
    let mut fsinfo = core::mem::MaybeUninit::<LfsFsinfo>::uninit();
    assert_ok(lfs_fs_stat(&mut lfs, &mut caches, fsinfo.as_mut_ptr()));
    let fsinfo = unsafe { fsinfo.assume_init() };
    assert_eq!(fsinfo.block_size, BLOCK_SIZE);
