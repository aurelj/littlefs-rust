...
fn test_move_nop() {
    ...
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, hi.as_ptr()));
    assert_ok(lfs_rename(&mut lfs, &mut caches, hi.as_ptr(), hi.as_ptr()));

    let hi_hi = path_bytes("hi/hi");
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, hi_hi.as_ptr()));
    ...
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, hi_hi_hi.as_ptr()));
    assert_ok(lfs_rename(
        &mut lfs,
        &mut caches,
        hi_hi_hi.as_ptr(),
...
fn test_move_file() {
    ...
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, path_bytes("a").as_ptr()));
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, path_bytes("b").as_ptr()));
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, path_bytes("c").as_ptr()));
    assert_ok(lfs_mkdir(&mut lfs, &mut caches, path_bytes("d").as_ptr()));
    ...
    let a_names =
        dir_entry_names(&mut lfs, &mut caches, &env.config as *const LfsConfig, "a").unwrap();
    assert_eq!(a_names.len(), 0);
    let c_names =
        dir_entry_names(&mut lfs, &mut caches, &env.config as *const LfsConfig, "c").unwrap();
...
fn test_move_state_stealing() {
    ...
    assert_ok(lfs_remove(&mut lfs, &mut caches, path_bytes("b").as_ptr()));
    assert_ok(lfs_remove(&mut lfs, &mut caches, path_bytes("c").as_ptr()));
    assert_ok(lfs_unmount(&mut lfs));
...
fn test_move_create_delete_same() {
    ...
    let n = lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"test.1");
    assert_eq!(n, 6);
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
    ...
    let n = lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"test.2");
    assert_eq!(n, 6);
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
    ...
    let n = lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"test.3");
    assert_eq!(n, 6);
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
    ...
    let _ = lfs_file_write(&mut lfs, &mut caches, fa.as_mut_ptr(), b"test.4");
    let _ = lfs_file_write(&mut lfs, &mut caches, fb.as_mut_ptr(), b"test.5");
    let _ = lfs_file_write(&mut lfs, &mut caches, fc.as_mut_ptr(), b"test.6");
    ...
    assert_ok(lfs_file_close(&mut lfs, &mut caches, fa.as_mut_ptr()));
    assert_ok(lfs_file_close(&mut lfs, &mut caches, fb.as_mut_ptr()));
    assert_ok(lfs_file_close(&mut lfs, &mut caches, fc.as_mut_ptr()));

    let names =
        dir_entry_names(&mut lfs, &mut caches, &env.config as *const LfsConfig, "/").unwrap();
...
fn test_move_create_delete_delete_same() {
    ...
    let n = lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"remove me");
    assert_eq!(n, 9);
    assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
...
fn test_move_create_delete_different() {
    ...
    assert_ok(lfs_mkdir(
        &mut lfs,
        &mut caches,
        path_bytes("dir.1").as_ptr(),
    ));
    assert_ok(lfs_mkdir(
        &mut lfs,
        &mut caches,
        path_bytes("dir.2").as_ptr(),
    ));
...
fn test_move_reentrant_file() {
    ...
    let result = run_powerloss_linear(
        ...
        |lfs, caches, config| {
            ...
            let err = lfs_rename(lfs, caches, path_src.as_ptr(), path_dst.as_ptr());
            if err != 0 {
...
fn test_move_fix_relocation() {
    ...
    for relocations in 0..4u32 {
        ...
        assert_eq!(
            lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), b"move me\0",),
            8
        );
        assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
        ...
        for (path, content) in [
            ...
            assert_eq!(
                lfs_file_write(&mut lfs, &mut caches, file.as_mut_ptr(), content,),
                7
            );
            assert_ok(lfs_file_close(&mut lfs, &mut caches, file.as_mut_ptr()));
        }
        ...
        {
            assert_eq!(
                lfs_file_write(&mut lfs, &mut caches, f.as_mut_ptr(), content,),
                7
            );
        }
        ...
        for f in &mut files {
            assert_ok(lfs_file_close(&mut lfs, &mut caches, f.as_mut_ptr()));
        }
        ...
        let mut idx = 0;
        loop {
            let n = lfs_dir_read(&mut lfs, &mut caches, dir.as_mut_ptr(), info.as_mut_ptr());
            assert!(n >= 0);
            if n == 0 {
                break;
            }
        ...
        for (path, expected) in [
            ...
            assert_eq!(
                lfs_file_read(&mut lfs, &mut caches, file.as_mut_ptr(), &mut buf[..7],),
                7
            );
