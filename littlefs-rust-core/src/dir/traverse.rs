    ...
    loop {
        ...
        match phase {
                    ...
                    && !(crate::tag::lfs_tag_id(tag) >= begin && crate::tag::lfs_tag_id(tag) < end)
                {
                    ...
                    if type3 == LFS_FROM_NOOP as u16 {
                        ...
                        while i < attr_count {
                            ...
                            res = if is_filter_cb {
                                let filtertag =
                                    &mut unsafe { &mut *stack[sp - 1].as_mut_ptr() }.tag;
                                dispatch_tag(
                                    lfs,
                                    caches,
                        ...
                        res = if is_filter_cb {
                            let filtertag = &mut unsafe { &mut *stack[sp - 1].as_mut_ptr() }.tag;
                            dispatch_tag(
                                lfs,
                                caches,
                                &mut |lfs, caches, tag, buffer| {
            ...
            TraversePhase::PopAndProcess => {
                ...
                diff = frame.diff;
                is_filter_cb = frame.is_filter_cb;
                disk = frame.disk;
