...
async fn read_magic_region<S: Storage>(lfs: &mut Lfs<S>, block: u32) -> Option<[u8; 8]> {
    let mut buf = [0u8; 24];
    lfs.storage
...
pub async fn assert_block_has_magic<S: Storage>(lfs: &mut Lfs<S>, block: u32) {
...
pub async fn assert_blocks_0_and_1_have_magic<S: Storage>(lfs: &mut Lfs<S>) {
    assert_block_has_magic(lfs, 0).await;
    assert_block_has_magic(lfs, 1).await;
}
